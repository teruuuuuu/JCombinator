...
public class OrParser<X> implements Parser<X> {
    ...
    @Override
    public ParseResult<X> parse(String input, ParserContext context) {
            ...
            case ParseResult.Failure<X> ffailure -> {
                switch (secondParser.parse(input, context)) {
                    ...
                    case ParseResult.Failure<X> sfailure -> {
                        return new ParseResult.Failure<>(context.newError(ffailure.context().lastError()).newError(sfailure.context().lastError()));
                    }
                }
            }
        }
